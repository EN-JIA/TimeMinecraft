...
public class MCY100NEWPLAINSBiome extends TimeModElements.ModElement {
	public static Biome biome;

	public MCY100NEWPLAINSBiome(TimeModElements instance) {
		super(instance, 2334);
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(12638463).setWaterColor(4159204).setWaterFogColor(329011)
						.withSkyColor(7972607).withFoliageColor(-16738048).withGrassColor(-16724992).build();
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(3f).scale(0.7f).temperature(0.5f)
						.downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("time:mcy_100_newplains"));
			}
		}
	}
