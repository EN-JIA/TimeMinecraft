...
public class MCY100HugeTreeStructure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
					...
					if ((random.nextInt(1000000) + 1) <= 200000) {
						...
						for (int a = 0; a < count; a++) {
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("time", "mcy100hugetree"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("mcy_100_huge_tree"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("time:mcy_100_huge_tree"), configuredFeature);
		}
	}
	...
	public static void addFeatureToBiomes(BiomeLoadingEvent event) {
		...
		if (new ResourceLocation("time:mcy_100_huge_forest").equals(event.getName()))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
