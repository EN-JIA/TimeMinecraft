...
package enchia.time.main.world.structure;
...
public class MCY100SmallTree3Structure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if ((random.nextInt(1000000) + 1) <= 200000) {
						...
						for (int a = 0; a < count; a++) {
							...
							if (blockAt.getBlock() == Minecraftyears100grassBlock.block)
								blockCriteria = true;
							if (!blockCriteria)
								continue;
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("time", "mcy100littletree3"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("mcy_100_small_tree_3"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("time:mcy_100_small_tree_3"), configuredFeature);
		}
	}
	...
	public static void addFeatureToBiomes(BiomeLoadingEvent event) {
		...
		if (new ResourceLocation("time:mcy_100_small_forest").equals(event.getName()))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
